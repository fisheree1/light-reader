...
test('completes the release user loop through the real Foliate Web engine', async ({
  ...
  await page.getByRole('button', { name: '显示高亮与批注' }).click();
...
test('creates a note through the consented local AI selection flow', async ({
  ...
  await contentFrame
    .getByText('这是 LightReader 自制的无版权测试内容。')
    .selectText();
  const aiButton = page.getByRole('button', { name: 'AI 助手', exact: true });
  await aiButton.click();

  const sidebar = page.getByRole('complementary', {
    name: '本地 AI 阅读助手',
  });
  await expect(sidebar.getByLabel('将发送的准确文本')).toHaveValue(
    '这是 LightReader 自制的无版权测试内容。',
  );
  await sidebar.getByLabel('任务').selectOption('custom');
  await sidebar
    .getByRole('textbox', { name: '自定义需求' })
    .fill('找出这段话最重要的一个信息');
  await sidebar.getByRole('button', { name: '发送给本地模型' }).click();
  await expect(sidebar.getByText('已生成独立 AI 草稿')).toBeVisible();
  await sidebar.getByRole('button', { name: '创建新笔记' }).click();

  await expect(page).toHaveURL(/\/notes\?noteId=/);
  await expect(
    page.getByRole('textbox', { name: '笔记标题', exact: true }),
  ).toHaveValue('自定义需求：AI 阅读草稿');
  await expect(
    page
      .getByLabel('笔记正文')
...
test('asks the current book a free-form question with a navigable citation', async ({
  page,
}) => {
  await page.goto('/settings');
  await page.getByLabel('启用本地 AI 助手').check();
  ...
  await page.getByRole('button', { name: '显示 AI 助手' }).click();

  const sidebar = page.getByRole('complementary', {
    name: '本地 AI 阅读助手',
  });
  await sidebar.getByLabel('向本书提问').fill('本书有哪些测试内容？');
  await sidebar.getByRole('button', { name: '检索本书并提问' }).click();

  await expect(sidebar.getByText('已生成独立 AI 草稿')).toBeVisible();
  const citation = sidebar.getByRole('button', { name: /依据 1 · 第一章/ });
