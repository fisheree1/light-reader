...
当前完成了 EPUB/PDF 导入、书架持久化和管理、基础阅读、高亮、轻量文字批注、独立笔记、本地全文搜索和数据库备份恢复闭环。书架可按书名、作者或标签搜索，按最近阅读、添加时间或标题排序，并支持收藏、标签和两种安全删除模式。阅读设置、位置、高亮、批注、版本化 Tiptap JSON 笔记和派生搜索索引均持久化到本地 SQLite，并可导出为 `.lightreader-backup` 后在兼容版本中恢复。
...
Tauri 文件删除采用应用受控隔离区：先原子重命名电子书目录和封面，再执行 SQLite 事务；事务失败会把文件移回原位置，事务成功后才移除隔离文件。任何目标路径都必须匹配应用生成的 `light-reader/books/<id>` 和 `light-reader/covers/<id>.<ext>`，不会删除用户提供的任意路径。

## 数据备份与恢复

设置页可以把书籍元数据、收藏和标签、阅读设置与进度、高亮、批注、笔记以及本地搜索索引导出为一个 `.lightreader-backup` 文件。备份只在 Tauri 桌面环境可用，完全本地执行，不调用外部 API。

导出使用 SQLite `VACUUM INTO` 生成一致性快照，不直接复制正在运行的 `light-reader.db`。导入会先检查固定归档结构、文件大小、SQLite 文件头、SHA-256、备份格式版本、数据库 Schema 版本、完整性、记录计数，以及每本书的受控 EPUB/PDF/封面路径与本机文件状态，验证完成后才显示“确认恢复”。恢复在单个 SQLite 事务中替换规范数据；约束、计数或写入失败会回滚，保留恢复前数据。
...
- 打开应用受控目录中的 EPUB；
- 嵌套目录展示与章节跳转；
- 按钮及左右方向键翻页；EPUB 还支持滚轮/触控板翻页和左右触控滑动；
...
- 导入应用受控目录中的 PDF，并由独立 `PdfEbookReader` 打开；
- 使用页码、页内进度和可选文本偏移范围组成的版本化 PDF locator；
- PDF 文本层选择、高亮、批注定位和全文查找；
- 只渲染可见页附近的三页窗口，活动页缓存最多五页；
- 限制 canvas 尺寸与像素量，文本缓存使用 32 页 LRU；
- 关闭或切换图书时取消渲染和文本层，销毁 loading task 与 worker。
- PDF Worker 使用本地打包资源的绝对地址，并由 PDF.js loading task 统一管理，避免 worker 与文档生命周期分离。
...
4. 打开 EPUB 或 PDF，从顶部工具栏打开 AI 侧边栏，或选中正文后点击“AI 助手”；
5. 使用“本书问答”直接输入问题，或在“处理选区”中选择预设任务/输入自定义需求；
6. 本书问答会在本机提取 EPUB 章节或 PDF 文本层、建立派生索引，只把最相关的有限片段发送给 Ollama；
7. 首次索引会显示读取、提取、切块和写入进度；可随时取消，也可用“重建本书 AI 索引”手动修复派生索引；
8. 生成结果先成为独立 AI 草稿；本书问答的引用可跳回章节或 PDF 页码，也可复制、丢弃或明确确认后创建新笔记；
9. 完成后可展开“本次运行详情”查看脱敏的工具调用状态，其中不包含书籍原文。

AI 默认关闭，只允许固定的本机 Ollama 地址。模型不能访问文件路径、SQL、
Repository、其他书籍或原始笔记；只读工具受单书授权、次数、文本总量和时效
限制，工具轨迹不记录原文。引用会验证对应原文位置和图书版本，但
仍需用户核对证据是否充分支持模型的结论。思考内容不会显示或持久化。关闭确认框、取消、超时
或失败都不会写入笔记。没有文本层的扫描版 PDF 暂不支持本书问答。

## 当前未实现

- 跨设备协作编辑
- 云同步（已完成架构与隐私评估，尚未接入服务）
- 语义向量检索、跨书研究 Agent 和学习工作流
