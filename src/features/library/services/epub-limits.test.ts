...
describe('EPUB resource limits', () => {
  it('validates the compressed EPUB file size without allocating it', () => {
    expect(() => {
      assertEpubFileSize(MAX_EPUB_FILE_SIZE);
    }).not.toThrow();
    expect(() => {
      assertEpubFileSize(MAX_EPUB_FILE_SIZE + 1);
    }).toThrow(expect.objectContaining({ code: 'EPUB_TOO_LARGE' }) as AppError);
  });
  ...
  it('rejects a selected ZIP entry with an excessive compression ratio', () => {
    ...
    ).toThrow(expect.objectContaining({ code: 'EPUB_TOO_LARGE' }));
