...
export function sha256WithoutWebCrypto(data: Uint8Array): string {
...
export class WebCryptoContentHasher implements ContentHasher {
  private readonly subtle: Pick<SubtleCrypto, 'digest'> | null;

  constructor(
    subtle: Pick<SubtleCrypto, 'digest'> | null = (
      globalThis.crypto as Omit<Crypto, 'subtle'> & {
        subtle?: SubtleCrypto;
      }
    ).subtle ?? null,
  ) {
  ...
  async sha256(data: Uint8Array): Promise<string> {
    if (!this.subtle) return sha256WithoutWebCrypto(data);
    const digest = await this.subtle.digest(
      'SHA-256',
